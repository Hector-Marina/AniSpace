...
```{r}
plotAniSpace(df,NIDs=c(13,70))
```

## Three individuals

```{r}
plotAniSpace(df,NIDs=c(13,40,41))
```
...
Using the function `filter.AniSpace()` allows us to filter the `AniSpace` object using the identification of individuals, specific time ranges and/or certain areas.

```{r}
df.ID.filt=filter.AniSpace(AniObj=df,NIDs=c(13:16))
```
...
df.Time.filt=filter.AniSpace(AniObj=df, TimeWindow=c(Tmin,Tmax))
```
...
```{r}
df.Area.filt=filter.AniSpace(AniObj=df,Area=as.character(10))
```
...
When filtering the `AniSpace` object using the function `filter.AniSpace()` we can consider an extension of the borders of the areas by using the option `soft.boundaries`:
```{r}
df.Area.filt2=filter.AniSpace(AniObj=df,Area=as.character(10), soft.boundaries = 100)
```
...
```{r}
plotAniSpace(df.inter,NIDs=1)
Inter=df.inter@Pos[[1]]$Inter
points(df.inter@Pos[[1]]$x[!Inter],df.inter@Pos[[1]]$y[!Inter],col="black" )
```

The spikes found beyond the designated areas prove the necessity of pre-processing this data by filtering the spikes registered by the system using the function `filter.spikes()` and clustering the positions to smooth the animals' walking paths using the function `cluster.AniSpace()`. However, these functions are not available for this vignette.  
