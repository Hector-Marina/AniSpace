...
#'
#' @description
#' `movement.AniSpace` calculates step distances, speeds, and turning angles for each individual. When requested, it applies k-means clustering to movement observations and summarizes the occurrence of each movement-behaviour cluster within the areas defined in `AniObj`.
#'
...
#' df.mov2 <- movement.AniSpace(df, k.means = TRUE, k.mov = 2)
...
movement.AniSpace=function(AniObj, k.means=FALSE, k.mov=NULL, k.max=10, verbose=TRUE) {
  ...
  if(k.means){
    ...
    mov.k=kmeans(m[,c("dist","speed","turn_angle")], centers=k.mov, iter.max = 10)
