...
#'
#' @description
#' `interpolate` interpolating individual temporal-spatial information.
#'
...
#'
#' @examples
#' # Interpolate individual trajectories using modified Akima interpolation
#' df.inter=interpolate(AniObj,method="makima",TRes=1,verbose=FALSE)
#'
#' # Compare the number of positions before and after interpolation
#' c(
#'   original=sum(vapply(AniObj@Pos,function(p) length(p$Time),numeric(1))),
#'   interpolated=sum(vapply(df.inter@Pos,function(p) length(p$Time),numeric(1)))
#' )
#'
#' # Inspect interpolated positions
#' Pos.inter=as.data.frame(df.inter)
#' head(Pos.inter[Pos.inter$Inter,])
#'
#'
#' @export

interpolate=function(AniObj, method="makima", TRes=1, TCap=120, tails=FALSE, verbose=TRUE){
  ...
  VAL=validate(AniObj)
  if(!VAL) stop("The interpolation results produced an invalid `AniObj` object.")
