...
#'
#' @description
#' `filter.AniSpace` filtering the individual temporal-spatial information.
#'
...
#' df.ID.filt=filter.AniSpace(df,NIDs=c(1:5))
#' df.ID.filt
#'
...
#' df.Time.filt=filter.AniSpace(df,TimeWindow=c(Tmin,Tmax))
#' df.Time.filt
#'
#' # Filter positions from the resting area
#' df.Area.filt=filter.AniSpace(df,Area=c(1:5))
#' df.Area.filt
#'
#'
#' @export

filter.AniSpace=function(AniObj, NIDs=NULL, IDs=NULL, TimeWindow=NULL, Area=NULL, soft.boundaries=0, verbose=TRUE) {
