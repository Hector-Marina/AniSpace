...
#'
#' @export
setMethod("merge", signature = c(x = "AniSpace", y = "AniSpace"), function(x, y, ...){
  ...
  names(xx@Pos)=xx@IDs
  if(ddS) message('Duplicated positions were found when merging. Obj1 (`x`) positions have been retained.')

  # Area Information
  n1=sapply(x@Area, `[[`, "ID") |> as.character()
  n2=sapply(y@Area, `[[`, "ID") |> as.character()
  n3=intersect(n1,n2)
  if(length(n3)>0) message('Duplicated area information was found when merging. Obj1 (`x`) information has been retained.')
